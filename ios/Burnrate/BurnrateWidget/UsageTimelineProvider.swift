...
struct UsageTimelineProvider: TimelineProvider {
    ...
    private func userID() -> String? {
        // Read from shared App Group UserDefaults (written by the iOS companion app).
        if let shared = UserDefaults(suiteName: appGroup)?.string(forKey: "supabaseUserId"),
           !shared.isEmpty {
            return shared
        }
        // Fallback: iCloud key-value store synced from the macOS app.
        return NSUbiquitousKeyValueStore.default.string(forKey: "supabaseUserId")
    }
